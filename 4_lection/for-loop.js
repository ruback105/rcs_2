...
// const count = [3, 2, "5", 4, null, "5", 120]; // 0 1 2 3 -> length = 4
...
for (let i = 0; i < mobilePhones.length; i += 1) {
  const mobilePhone = mobilePhones[i];

  const { screenSize, price } = filters;

  if (
    mobilePhone.specifications.screenSize <= screenSize &&
    mobilePhone.price <= price
  ) {
    matchingPhones.push(mobilePhone);
  }
}

console.log(matchingPhones);
...
//   if (mobilePhone.price <= 700) {
//     cheapPhones.push(mobilePhone);
//   } else {
//     pricyPhones.push(mobilePhone);
//   }
// }

// console.log(cheapPhones);
// console.log(pricyPhones);
