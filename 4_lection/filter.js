...
const filteredPhones = mobilePhones.filter((phone) => {
  const cameraFeatures = phone.specifications.camera.features;
  const hasFeature = cameraFeatures.includes(filter.feature);

  if (phone.price < filter.price && hasFeature) {
    return true;
  } else {
    return false;
  }
